...
public class PlayerAttack : MonoBehaviour
{
    ...
    void WeaponShoot() {
        ...
        // if we press and hold left mouse click and Time  > nextTimeToFire

        if(weapon_Manager.GetCurrentSelectedWeapon().fireType  == WeaponFireType.MULTIPE) {

            if(Input.GetMouseButtonDown(0) && Time.time > nextTimeToFire) {

                    nextTimeToFire = Time.time + 1f/fireRate; // fireRate;

                    weapon_Manager.GetCurrentSelectedWeapon().ShootAnimation();
                    BulletFired();

            }
            // if we have a regular weapon that shoot once
    ...
    void ZoomInAndOut() {
        ...
        if(weapon_Manager.GetCurrentSelectedWeapon().weapon_Aim == WeaponAim.AIM) {
            ...
            // when the right mouse click and hold
